...
:run
powershell.exe -NoLogo -NoProfile -ExecutionPolicy Bypass -Command "$l=[IO.File]::ReadAllLines($env:CSV_TOOL_SELF,[Text.Encoding]::ASCII);$i=[Array]::IndexOf($l,'//__CSHARP__');$s=[string]::Join([Environment]::NewLine,$l[($i+1)..($l.Length-1)]);Add-Type -TypeDefinition $s -Language CSharp;exit [CsvTool]::Run($env:CSV_TOOL_ARGS)"
exit /b %ERRORLEVEL%
...
public static class CsvTool
{
    ...
    private static readonly Dictionary<string, string> OutputNames =
    ...
    public static int Run(string packedArguments)
    {
        try
        {
            ...
            string outputBaseName = SelectOutputName();
            string outputPath = ResolveOutputPath(inputs[0].Path, outputBaseName);
    ...
    private static string SelectOutputName()
    {
        ...
        foreach (KeyValuePair<string, string> option in OutputNames)
            Console.WriteLine("  " + option.Key + ") " + option.Value + "_" + date + ".csv");

        while (true)
        {
            Console.Write("\u756A\u53F7 (1-3): ");
            string key = Console.ReadLine();
            string name;
            if (key != null && OutputNames.TryGetValue(key, out name)) return name;
        }
    }

    private static string ResolveOutputPath(string firstInputPath, string baseName)
    {
        string date = DateTime.Now.ToString("yyyyMMdd");
        string outputDirectory = Path.Combine(
            Path.GetDirectoryName(firstInputPath), "output_" + date);
    ...
    private static long AppendDataRows(InputInfo input, TextWriter writer)
    {
        ...
        {
            reader.ReadRow(); // \u30D8\u30C3\u30C0\u30FC\u306F\u51FA\u529B\u6E08\u307F\u306A\u306E\u3067\u8AAD\u307F\u98DB\u3070\u3059\u3002

            string[] row;
            while ((row = reader.ReadRow()) != null)
            {
                rowNumber++;
                ValidateColumnCount(input, row, rowNumber);
