...
今回の①②中心の統合・両方の決済ステータス判定・処理済みだけ削除するサンプルは、[業務サンプルの使い方](PAYMENT-GUIDE.md)を参照してください。本番用JSONは `configs/production/settings.json`、サンプル用は `configs/sample/settings.json` です。配布ZIPはサンプル設定を同梱します。下記は汎用設定の説明です。
...
64ビットWindows、Windows PowerShell 5.1の.NET Framework/WPF、WebView2 Runtimeを使います。ZIPを展開し、`src/web/lib`を含む一式をそのまま配置します。通常起動は`ReaderDataViewer.vbs`、コンソールで理由を読みたいときは`ReaderDataViewer.cmd`。WebView2のDLLは同梱ですが、端末のRuntimeや組織の実行制御は環境ごとに確認してください。

配布フォルダーには起動入口、settings.json、本README、src/web/lib、補足docs、任意の合成CSV4本、空のoutputフォルダー、配布物一覧が入ります。実運用の台帳、ログ、未送信変更は含めません。入力例はtableA/B/C各1000行、台帳は有効な主キーを持つ990行です。例えば検索値00000001は候補を表示できますが、実運用設定では必ず対象CSVの検索値を使ってください。

開発用リポジトリには別に`build.bat/tools/tests/design`があります。`build.bat package -Theme win98 -RunTests`で新しい配布フォルダーとZIPを作ります。既存の配布物を上書きしません。`build.bat compile`と`build.bat test`は開発用の検査です。配布先の設定作者は前述のValidateOnly/RunUpdateを使えます。

補足：[設定の早見表](docs/settings.md)、[共有運用](docs/shared-ledger.md)、[配布の作成](docs/design-build.md)。以下の索引は、これらを開かなくても設定の各値へ辿れるよう本READMEに含めています。
