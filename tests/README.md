...
## 2026-09-11 配置・移動・保存列名

追加修正の再現と配布実窓の結果は [RELOCATION-20260911.md](RELOCATION-20260911.md)。`Test-Review.ps1` に正規の「確認状態」列・未送信保存先の一式移動・公開記録の検査を追加した。今回追加していた旧列名・旧保存先の互換処理は撤回・除去した。個別テストの再生成後は `python tests/public_results.py` で検査記録の利用者パスを匿名化し、`python tests/public_results.py --check` で確認できる。標準回帰入口とレビュー梱包もこれを実行する。レビュー梱包には標準ライブラリだけのPython 3を使う。通常ビルドの依存関係は変更していない。
