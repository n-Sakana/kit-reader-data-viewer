...
async function main(){let app,modal;try{
  ...
  const masked=['applicationDate','applicationNumber','cardNumber','applicantName','birthDate','qualification','office','remarks','plan'];
  const deleteOnce=async()=>{
    await click(app,'#b-del');await visible('v-del');
    await click(modal,'#v-del [data-command=execute]');
  ...
  check('fixed DOM loaded',await app.evaluate(`document.querySelectorAll('.win [data-bind]').length===17 && !document.querySelector('.win [data-section-type]')`));
  ...
  await ready();await waitFor(app,`document.querySelector('[data-judgment=paymentStatus] .ok').textContent==='済'`,10000,'paid');
  ...
  check('sent changes clear the display',await app.evaluate(`document.querySelector('[data-bind=applicationNumber]').textContent===''`));
  await search(info.noAppKey);await waitFor(app,`document.querySelector('[data-judgment=paymentStatus] .ok').textContent==='済'`,10000,'paid without APP');
  check('paid without APP remains searchable',await app.evaluate(`document.querySelector('[data-bind=applicationNumber]').textContent!=='' && document.querySelector('[data-bind=applicantName]').textContent===''`));
  await capture(app,'main-no-application');
  await search(info.missingKey);await waitFor(app,`document.querySelector('[data-judgment=paymentStatus] .ok').textContent==='未決済'`,10000,'no payment');
  ...
  await click(modal,'#v-set [data-command=execute]');await ready();
  check('settings save retains compact bindings',!!JSON.parse(fs.readFileSync(info.settings,'utf8').replace(/^\uFEFF/,'' )).screen.bindings);
  ...
  await click(modal,'#v-out [data-command=execute]');await ready();
  ...
  for(const [button,id,count] of [['#b-upd','v-upd',100],['#b-del','v-del',99]]){
    ...
    await click(modal,`#${id} [data-command=execute]`);
